...
Il progetto resta un laboratorio locale. Il collegamento a un nodo non deve trasformare l’app in una shell remota o in un agente con privilegi impliciti. L’identità del nodo, il trasporto e le capability ammesse devono essere dichiarati prima dell’attivazione.

## Contratto del nodo pubblico ZDOS

Il binding previsto è intenzionalmente ristretto al nodo pubblico reale `core-01`, che espone un’identità verificabile e operazioni allowlist. Il profilo autorizza soltanto `node.status`, `evidence.append` e `manifest.preview`; non autorizza shell remota, esecuzione arbitraria, accesso libero al filesystem, socket generici, gestione di segreti o modifica di capability.
...
Il progetto espone il solo nodo pubblico `core-01` con stato `IDENTIFIED / UNLINKED`: il client può leggere un heartbeat pubblico HTTPS, ma non attiva alcun collegamento di controllo remoto. Il trasporto resta `not-configured`.
...
La firma non invia dati, non identifica l’utente e non sostituisce una verifica crittografica. Nel nodo pubblico, una futura implementazione potrà affiancare a `ZTRACE` una vera attestazione verificata, senza riutilizzare questo valore come credenziale.
